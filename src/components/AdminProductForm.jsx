...
export default function AdminProductForm({ product, onSave, onClose }) {
  ...
  const [formData, setFormData] = useState(() => ({
    ...
    tag: product?.tag || '',
    tagColor: product?.tagColor || product?.tag_color || 'pink',
    devices: product?.devices || '',
    resolution: product?.resolution || '',
    packageDetails: product?.packageDetails || product?.package_details || '',
    subDetail: product?.subDetail || product?.sub_detail || '',
    priceLabel: product?.priceLabel || product?.price_label || 'ลูกค้า',
    price: product?.price || '',
    hasSecondPrice: Boolean(product?.hasSecondPrice ?? product?.has_second_price),
    secondPriceLabel: product?.secondPriceLabel || product?.second_price_label || 'ร้าน',
    secondPrice: product?.secondPrice || product?.second_price || '',
    priceUnit: product?.priceUnit || '฿',
    pricePeriod: product?.pricePeriod || product?.price_period || '',
    icon: product?.icon || APP_ICONS.iqiyi,
    orderLink: product?.orderLink || product?.order_link || '',
    inStock: product?.inStock !== false && product?.in_stock !== false,
    stockStatus: product?.stockStatus || product?.stock_status || (product?.inStock === false || product?.in_stock === false ? 'out_of_stock' : 'ready'),
    stockStatusText: product?.stockStatusText || product?.stock_status_text || ''
  }));
      ...
      <div className="bg-white rounded-3xl max-w-2xl sm:max-w-3xl w-full p-5 sm:p-8 shadow-2xl border border-pink-100 my-auto relative max-h-[92vh] flex flex-col">
        ...
        <form onSubmit={handleSubmit} className="overflow-y-auto pr-1 space-y-4 text-xs sm:text-sm">
          ...
          <div className="p-4 bg-pink-50/60 rounded-2xl border border-pink-200/80 space-y-3">
            ...
            {/* Quick Bulk Status for All Rates/Accounts */}
            {prices.length > 1 && (
              <div className="flex items-center justify-between bg-white/90 p-2.5 rounded-xl border border-pink-100 flex-wrap gap-2">
                <span className="text-[11px] font-bold text-slate-700 flex items-center gap-1">
                  <span>⚡ ปรับสถานะทุกเรท ({prices.length} เรท) พร้อมกัน:</span>
                </span>
                <div className="flex items-center gap-1.5 flex-wrap">
                  <button
                    type="button"
                    onClick={() => setPrices(prices.map((p) => ({ ...p, status: 'ready' })))}
                    className="px-2.5 py-1 text-[11px] font-bold bg-emerald-50 hover:bg-emerald-100 text-emerald-800 border border-emerald-300 rounded-lg cursor-pointer transition-all active:scale-95"
                  >
                    🟢 พร้อมส่งทั้งหมด
                  </button>
                  <button
                    ...
                    className="px-2.5 py-1 text-[11px] font-bold bg-amber-50 hover:bg-amber-100 text-amber-800 border border-amber-300 rounded-lg cursor-pointer transition-all active:scale-95"
                  >
                    🟠 รอกดทั้งหมด
                  </button>
                  <button
                    ...
                    className="px-2.5 py-1 text-[11px] font-bold bg-rose-50 hover:bg-rose-100 text-rose-800 border border-rose-300 rounded-lg cursor-pointer transition-all active:scale-95"
                  >
                    🔴 หมดทั้งหมด
                  </button>
                </div>
